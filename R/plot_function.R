...
plot_function_1_dynml=function(EQ1_4,begin_year=start((EQ1_4[["residuals"]]))[1],begin_month=start((EQ1_4[["residuals"]]))[2],end_year=end((EQ1_4[["residuals"]]))[1],end_month=end((EQ1_4[["residuals"]]))[2],new_var= rep(0,length(EQ1_4[["residuals"]])),p =1  ){
  ETK = EQ1_4[["residuals"]]
  if(is(ETK)[1]=="ts"){
    ...
    plot(k,ylim=c(1.5*min(EQ1_4[["residuals"]]),max(k)*1.1),xlim=c(begin_year+(begin_month/12),end_year+(end_month/12)),col="black",type = "l",lwd=1)
    lines((EQ1_4[["fitted.values"]]),col="red",lwd=1)   ## Pinta los valores ajustados sobre el gr?fico
    points((EQ1_4[["residuals"]]),type="l",col="darkblue")
    points(qt,type="l",col="green")
    abline(h=0,pch=2,lty=2,col="gray50")        ## Pinta la lÃ???nea de referencia en cero
    abline(v=c(seq(start(EQ1_4[["residuals"]])[1],end(EQ1_4[["residuals"]])[1]+(end(EQ1_4[["residuals"]])[2]/12),1/EQ1_4[["frequency"]][1])),pch=2,lty=2,col="gray50")
  } else{
    qt = ts(new_var*p,start=start(EQ1_4[["residuals"]]),end=end(EQ1_4[["residuals"]]),frequency=frequency(EQ1_4[["residuals"]]))
    k = (EQ1_4[["residuals"]]+EQ1_4[["fitted.values"]])
    plot(k,ylim=c(1.5*min(EQ1_4[["residuals"]]),max(k)*1.1),xlim=c(start(EQ1_4[["residuals"]]),end(EQ1_4[["residuals"]])),col="black",type = "l",lwd=1)
